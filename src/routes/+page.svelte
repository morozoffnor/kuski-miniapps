<script>
	import { botUser } from '../stores/userStores.ts';
	import ItemCard from '$lib/Inventory/ItemCard.svelte';

	let items = []
	let inventoryLoaded = false
	$: if ($botUser.items != undefined) {
		items = $botUser.items
...
<div class="container h-full flex justify-center items-start">
	{#if inventoryLoaded}
		...
		{#if items.length > 0}
			<section class="grid grid-cols-2 md:grid-cols-3 gap-4 mt-5 mb-5">
				{#each items as item}
					<div>
						<ItemCard item={item} />
					</div>
...
<!--		<h2 class="h2">Welcome to Kuski.</h2>-->
<!--		&lt;!&ndash; Animated Logo &ndash;&gt;-->
<!--		<figure>-->
